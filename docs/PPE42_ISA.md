...
- Only **CR0 exists**
- CR1 through CR7 are reserved and are not implemented
- 4 bits:
...
- Do not enable LLVM's `crbits` feature for PPE42
- Do not select any CR-logical instruction or extended mnemonic
- Generate comparisons and conditional branches using CR0 directly
...
- divide instructions
- **All condition-register logical instructions and extended mnemonics:**
  `crand`, `crandc`, `crclr`, `creqv`, `crmove`, `crnand`, `crnor`,
  `crnot`, `cror`, `crorc`, `crset`, and `crxor`
...
mulli mullw mulhw (XM only) mulhwu (XM only)

Variant qualification:

- PPE42X and PPE42XM implement `mulli` and `mullw`.
- PPE42XM additionally implements `mulhw`, `mulhw.`, `mulhwu`, and `mulhwu.`.
- Original PPE42 implements none of those four full-word multiply families;
  it provides the halfword operations `mullhw` and `mullhwu` instead.
...
### 6.5 Instruction allowlist reference

These are human-readable reference lists of mnemonics as printed by
`llvm-objdump`, including aliases and record forms. The CI-enforced copies live
in `appsource/check_ppe42_disassembly.py`; changes to supported instructions
must update both places in the same review. PPE42X inherits PPE42; PPE42XM
inherits PPE42 and PPE42X.

<!-- PPE42_REFERENCE_BEGIN -->
```text
...
bng bnl bne bso bns bun
bnbw bnbwl bnbwi bnbwil
...
<!-- PPE42X_REFERENCE_BEGIN -->
```text
mulli mullw mullw. mullwo mullwo.
rldicl rldicl. rldicr rldicr. rldimi rldimi. slvd srvd
```
<!-- PPE42X_REFERENCE_END -->

<!-- PPE42XM_REFERENCE_BEGIN -->
```text
mulhw mulhw. mulhwu mulhwu.
```
<!-- PPE42XM_REFERENCE_END -->

---

## 7. INSTRUCTION BEHAVIOUR NOTES

### 7.1 Condition-register and conditional-branch encoding

`bt` is an extended mnemonic for the supported `bc` instruction; changing its
printed spelling to `bc` does not change the machine instruction. For PPE42,
the `bc`, `bcctr`, and `bclr` architecture notes require instruction bits
11:13 to be `000`, because only CR0 is implemented. Consequently `bt` and
`bf` operands must select a CR0 bit (`cr_bit` 0 through 3), and explicit `bc`
forms must have a BI value that produces the same required encoding.

LLVM must reserve CR1 through CR7 (including their CR-bit subregisters) for
PPE42. For example, `bt 2, target` is valid, while `bt 6, target` selects
CR1.EQ, sets a prohibited encoding bit, and raises an illegal-instruction
exception.

`mtcr0 RS` is an extended mnemonic for the legacy encoding `mtcrf 128, RS`.
That exact `mtcrf` form is supported: instruction bit 11 must be zero and the
FXM field in bits 12:19 must equal 128 (`0x80`), selecting only CR0. Every
other FXM value is unsupported, as are `mtcr` (`mtcrf 255, RS`) and the newer
`mtocrf` form with bit 11 set.
...
- force the PowerPC `crbits` feature off for PPE42 at every optimization level;
  otherwise LLVM can select unsupported `crnot`, `crand`, and related ops
- add:
  - VDR load/store
  - fused compare-branch
  - lsku/stsku
